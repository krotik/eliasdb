...
func TestSimpleGraphStorage(t *testing.T) {
	...
	dgs, err := graphstorage.NewDiskGraphStorage(GRAPHMANAGER_TEST_DBDIR3)
	if err != nil {
		t.Error(err)
		return
	}
	...
	keyAttrs := PREFIX_NS_ATTRS + edge.Key()
	...
	keyAttr := PREFIX_NS_ATTR + edge.Key() + gm.nm.Encode32(data.EDGE_END2_KEY, false)
	...
	specMap, err := nodeTree.Get([]byte(PREFIX_NS_SPECS + node2.Key()))
	...
	edge.SetAttr(data.EDGE_END1_KEY, node3.Key())
	...
	edge2.SetAttr(data.EDGE_END1_KEY, node1.Key())
	edge2.SetAttr(data.EDGE_END1_KIND, node1.Kind())
	edge2.SetAttr(data.EDGE_END1_ROLE, "node1")
	edge2.SetAttr(data.EDGE_END1_CASCADING, true)

	edge2.SetAttr(data.EDGE_END2_KEY, node3.Key())
	edge2.SetAttr(data.EDGE_END2_KIND, node3.Kind())
	edge2.SetAttr(data.EDGE_END2_ROLE, "node2")
	edge2.SetAttr(data.EDGE_END2_CASCADING, false)
	...
	specMap, err = nodeTree.Get([]byte(PREFIX_NS_SPECS + node1.Key()))
	if err != nil || specMap == nil {
		t.Error("Unexpected result:", specMap, err)
	}
	...
	specMap, err = nodeTree.Get([]byte(PREFIX_NS_SPECS + node3.Key()))
	if err != nil || specMap == nil {
		t.Error("Unexpected result:", specMap, err)
		return
	}
	...
	edge2.SetAttr(data.NODE_KEY, "def")
...
func TestSimpleGraphStorageErrorCases(t *testing.T) {
	...
	edge.SetAttr(data.EDGE_END2_KEY, node2.Key())
	edge.SetAttr(data.EDGE_END2_KIND, "xxx")
	edge.SetAttr(data.EDGE_END2_ROLE, "node2-")
	edge.SetAttr(data.NODE_NAME, "Edge name")
	...
	edge.SetAttr(data.EDGE_END2_KIND, node2.Kind())
	edge.SetAttr(data.EDGE_END1_KIND, "xxx")
	...
	edge.SetAttr(data.EDGE_END1_KIND, node1.Kind())
	edge.SetAttr(data.EDGE_END1_KEY, "xxx")
	...
	edge.SetAttr(data.EDGE_END1_KEY, node1.Key())
	edge.SetAttr(data.EDGE_END2_KEY, "xxx")
	...
	sm := gm.gs.StorageManager("main"+"myedge"+STORAGE_SUFFIX_EDGES_INDEX, false)
	...
	sm.(*storage.MemoryStorageManager).AccessMap[2] = storage.ACCESS_INSERT_ERROR
	...
	sm = gm.gs.StorageManager("main"+"mykind"+STORAGE_SUFFIX_NODES, false)
	sm.(*storage.MemoryStorageManager).AccessMap[1] = storage.ACCESS_CACHE_AND_FETCH_ERROR
	...
	sm = gm.gs.StorageManager("main"+"myedge"+STORAGE_SUFFIX_EDGES, false)
	sm.(*storage.MemoryStorageManager).AccessMap[1] = storage.ACCESS_CACHE_AND_FETCH_ERROR
	...
	sm = gm.gs.StorageManager("main"+"mynewkind"+STORAGE_SUFFIX_NODES, false)
	sm.(*storage.MemoryStorageManager).AccessMap[1] = storage.ACCESS_CACHE_AND_FETCH_ERROR
	...
	sm = gm.gs.StorageManager("main"+"myedge"+STORAGE_SUFFIX_EDGES_INDEX, false)
	sm.(*storage.MemoryStorageManager).AccessMap[5] = storage.ACCESS_INSERT_ERROR
	...
	sm = gm.gs.StorageManager("main"+edge.Kind()+STORAGE_SUFFIX_EDGES_INDEX, false)
	sm.(*storage.MemoryStorageManager).AccessMap[1] = storage.ACCESS_CACHE_AND_FETCH_ERROR
	...
	sm = gm.gs.StorageManager("main"+edge.End2Kind()+STORAGE_SUFFIX_NODES, false)
	...
	sm = gm.gs.StorageManager("main"+edge.End2Kind()+STORAGE_SUFFIX_NODES, false)
	sm.(*storage.MemoryStorageManager).AccessMap[loc] = storage.ACCESS_FREE_ERROR
	...
	sm = gm.gs.StorageManager("main"+edge.End1Kind()+STORAGE_SUFFIX_NODES, false)
	sm.(*storage.MemoryStorageManager).AccessMap[loc] = storage.ACCESS_UPDATE_ERROR
	...
	_, loc, _ = nodeattTree.GetValueAndLocation([]byte(PREFIX_NS_ATTRS + edge.End1Key()))
	...
	sm = gm.gs.StorageManager("main"+edge.Kind()+STORAGE_SUFFIX_EDGES, true)
	sm.(*storage.MemoryStorageManager).AccessMap[11] = storage.ACCESS_INSERT_ERROR
	...
	sm = gm.gs.StorageManager("main"+edge.Kind()+STORAGE_SUFFIX_EDGES, false)
	sm.(*storage.MemoryStorageManager).AccessMap[11] = storage.ACCESS_CACHE_AND_FETCH_ERROR
	...
	sm.(*storage.MemoryStorageManager).AccessMap[11] = storage.ACCESS_FREE_ERROR
	...
	sm = gm.gs.StorageManager("main"+edge.Kind()+STORAGE_SUFFIX_EDGES_INDEX, false)
	sm.(*storage.MemoryStorageManager).AccessMap[2] = storage.ACCESS_FREE_ERROR
...
func TestEdgeOperations(t *testing.T) {
	...
	sm := gm.gs.StorageManager("main"+node1.Kind()+STORAGE_SUFFIX_NODES, false)
	sm.(*storage.MemoryStorageManager).AccessMap[1] = storage.ACCESS_CACHE_AND_FETCH_ERROR
	...
	_, loc, _ := tree.GetValueAndLocation([]byte(PREFIX_NS_SPECS + node1.Key()))
	...
	if !data.NodeCompare(edges[0], edge2, []string{data.NODE_KEY, data.NODE_KIND,
		data.EDGE_END1_KEY, data.EDGE_END1_KIND, data.EDGE_END1_ROLE, data.EDGE_END1_CASCADING,
		data.EDGE_END2_KEY, data.EDGE_END2_KIND, data.EDGE_END2_ROLE, data.EDGE_END2_CASCADING}) {

		t.Error("Edges should match:", edge2, edges[0])
		return
	}

	if !data.NodeCompare(nodes[0], node2, []string{data.NODE_KEY, data.NODE_KIND}) {
	...
	if !data.NodeCompare(nodes2[0], node1, []string{data.NODE_KEY, data.NODE_KIND}) {
		t.Error("Nodes should match:", node1, nodes2[0])
		return
	}

	if !data.NodeCompare(edges2[0], edge2, []string{data.NODE_KEY, data.NODE_KIND}) {
	...
	if !data.NodeCompare(nodes2[0], node1, []string{data.NODE_KEY, data.NODE_KIND, data.NODE_NAME}) {
		t.Error("Nodes should match:", node1, nodes2[0])
		return
	}

	if !data.NodeCompare(edges2[0], edge2, []string{data.NODE_KEY, data.NODE_KIND, data.NODE_NAME}) {
